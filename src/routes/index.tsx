...
const skills = [
  ...
  { name: "Tìm kiếm thông tin học thuật", level: 92, use: "Nghiên cứu, viết tiểu luận, báo cáo." },
  { name: "Đánh giá độ tin cậy của nguồn", level: 90, use: "Kiểm chứng thông tin trước khi trích dẫn." },
  { name: "Viết prompt hiệu quả", level: 93, use: "Khai thác AI hỗ trợ học tập & sáng tạo." },
  { name: "Làm việc nhóm trực tuyến", level: 88, use: "Điều phối dự án, phân công, deadline." },
  { name: "Sáng tạo nội dung số bằng AI", level: 90, use: "Video, infographic, thuyết trình số." },
  { name: "Sử dụng AI có trách nhiệm", level: 95, use: "Học thuật liêm chính, phản biện thông tin." },
  { name: "Tự đánh giá & cải thiện bản thân", level: 91, use: "Phản tư, đặt mục tiêu học tập." },
];
...
function PortfolioPage() {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
          ...
          <div className="reveal space-y-4">
            <div className="rounded-3xl border border-border bg-card p-6 shadow-card">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {["Công nghệ số", "Khám phá AI", "Sáng tạo nội dung", "Quản lý dữ liệu", "Làm việc nhóm", "Học thuật"].map((t) => (
      ...
      {/* ================= SKILLS ================= */}
      <Section id="ky-nang" eyebrow="Skills" title="Bảng tổng hợp kỹ năng đạt được" subtitle="Những năng lực số đã hình thành qua quá trình thực hiện Portfolio.">
        <div className="grid gap-5 md:grid-cols-2">
          {skills.map((s) => (
            <div key={s.name} className="reveal rounded-2xl border border-border bg-card p-5 shadow-card card-hover">
              <div className="flex items-center justify-between">
                <h3 className="font-bold">{s.name}</h3>
                <span className="rounded-full bg-secondary/20 px-2.5 py-1 text-xs font-bold text-secondary-foreground">{s.level}%</span>
              </div>
              <div className="mt-3 h-2 overflow-hidden rounded-full bg-muted">
                <div className="h-full gradient-primary transition-all" style={{ width: `${s.level}%` }} />
              </div>
              <p className="mt-3 text-sm text-muted-foreground"><b>Ứng dụng:</b> {s.use}</p>
            </div>
          ))}
        </div>
      </Section>
